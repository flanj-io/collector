...
| `spec-v1.yaml`, `spec-v2.yaml` | the mock provider's OpenAPI specs (live-vs-spec validation; v1→v2 version-diff) | `internal/drift/drift_test.go`, `processor/flanjdrift/speccache_test.go` (test fixtures only — they were baked into the image at `/etc/flanj/` until 2026-09-02, where nothing read them) |
