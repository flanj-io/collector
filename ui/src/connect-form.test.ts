...
describe('connect form seeding (background polls must never clobber typed text)', () => {
  it('seeds every field when nothing is touched', () => {
    const next = applySeed(
      { org: '', collectorName: '', name: '', email: '', localUrl: '' },
      seededValues(state, undefined, 'http://origin'),
      untouched(),
      false
    );
    expect(next).toEqual({ org: 'Acme Consumer Ltd', collectorName: 'prod-eu', name: 'Dana', email: 'ops@acme.example', localUrl: 'http://collector:5335' });
  });
  ...
  it('a touched field keeps the user text across a background seed; pristine fields still fill', () => {
    ...
    const next = applySeed(
      { org: '', collectorName: '', name: '', email: 'typing@partial', localUrl: '' },
      seededValues(state, undefined, 'http://origin'),
      touched,
      false
    );
  ...
  it("a field typed-then-cleared re-prefills on the next background seed — unless it is focused", () => {
    ...
    const cleared = applySeed(
      { org: 'Acme Consumer Ltd', collectorName: '', name: '', email: '', localUrl: '' },
      seededValues(state, undefined, 'http://origin'),
      touched,
      false
    );
  ...
  it('force (an explicit user action) overrides touched fields', () => {
    const touched = { org: true, collectorName: true, name: true, email: true, localUrl: true };
    const next = applySeed(
      { org: 'x', collectorName: 'v', name: 'y', email: 'z@z', localUrl: 'w' },
      seededValues(state, undefined, 'http://origin'),
      touched,
      true
    );
  ...
  it('null state falls back to defaultOrg + origin, still honoring touched', () => {
    const next = applySeed(
      { org: '', collectorName: '', name: '', email: 'kept@typed', localUrl: '' },
      seededValues(null, 'Health Org', 'http://origin'),
      { ...untouched(), email: true },
      false
    );
    expect(next).toEqual({ org: 'Health Org', collectorName: '', name: '', email: 'kept@typed', localUrl: 'http://origin' });
  });
});
