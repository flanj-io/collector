...
describe('the flag sheet is the modal it claims to be', () => {
  ...
  it('keeps focus inside while a create is in flight (the disclosure toggle is the one live control)', async () => {
    // A POST that never answers keeps `busy` set: textarea, the Who can open it
    // radios and field, and both buttons are disabled, so the trap has a single
    // control to wrap onto — itself. The domain field is filled first: since
    // thread-domain-gate a create with it empty shows a guard and posts nothing,
    // and that click would never reach the in-flight state.
