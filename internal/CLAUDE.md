...
| `drift` | live-vs-spec (kin-openapi) + version-diff (oasdiff). Technical adherence only. **Token-aware:** runs after the redaction floor, so schema errors whose offending scalar carries a `⟦REDACTED:…⟧` token are SKIPPED (redacted = unknown, not violated; scalar-only so container errors still fire). | `drift_test.go` (golden call + two breaking findings) + `drift_token_test.go` (redacted values never drift; genuine drift next to tokens still fires) |
