...
// on a status the contract DECLARES (the code or its NXX range, not `default`)
// becomes one finding too. The error is a route miss (or a request that could
// not be reconstructed). A response the document does not otherwise describe —
// an undeclared status, a default-only status answered in a foreign media type,
// a body that will not decode — is neither a finding nor an error HERE: it
// comes back as no findings, which is why the processor stamps off
// JudgeLiveVsSpec instead.
