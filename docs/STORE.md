...
- **The store pod's own write is queued and retried too** (2026-09-07): the
  ...
  sections, on by default — 64 MiB in memory, rejecting when full, backoff up
  to 15 minutes. A front's 5-minute retry therefore covers the hop, and the
  store pod covers the database: a batch it has accepted survives postgres (or
  a locked sqlite file) being away for a quarter of an hour, and a full queue
  hands the batch back to the front (503) rather than blocking. Same
