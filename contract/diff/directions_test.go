...
func wantOne(t *testing.T, got []Change, class Class, rule, fieldPath string) Change {
	t.Helper()
	if len(got) != 1 {
		t.Fatalf("got %d changes, want exactly 1:\n%s", len(got), dump(got))
	}
	c := got[0]
	if c.Class != class || c.Rule != rule || c.FieldPath != fieldPath {
		t.Fatalf("got %s %s at %q, want %s %s at %q", c.Class, c.Rule, c.FieldPath, class, rule, fieldPath)
	}
	return c
}
...
func TestTypeDirection_Cells(t *testing.T) {
	cases := []struct {
		...
		{name: "input widened (string -> [string,null]) is the one additive cell",
			...
			class: ClassNonBreaking, rule: RuleInputTypeWidened,
		...
		{name: "input narrowed ([integer,string] -> integer): a caller sending the dropped type now fails",
			...
			class: ClassBreaking, rule: RuleInputTypeNarrowed,
		...
		{name: "input changed (integer -> string): a swap",
			...
			class: ClassBreaking, rule: RuleInputTypeChanged,
		...
		{name: "output widened (number -> [number,string]; apify logo.height): the consumer may receive a type it never handled",
			...
			class: ClassBreaking, rule: RuleOutputPropertyTypeWidened,
		...
		{name: "output narrowed ([null,string] -> string): the null branch is dead code — the response-enum posture, applied to a type member",
			...
			class: ClassBreaking, rule: RuleOutputPropertyTypeNarrowed,
		...
		{name: "output changed (number -> string)",
			...
			class: ClassBreaking, rule: RuleOutputPropertyTypeChanged,
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			c := wantOne(t, got, tc.class, tc.rule, path)
...
func TestInputPropertyRemoved_RequiredVsOptional(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "required removed", wasRequired: true, class: ClassBreaking, rule: RuleInputRequiredPropertyRemoved, detailAbsent: true},
		{name: "optional removed, new schema additionalProperties:false (aave get_apy_history.reserve): callers still sending it now fail",
			newTail: `,"additionalProperties":false`, class: ClassBreaking, rule: RuleInputOptionalPropertyRemoved, detailHas: "additionalProperties: false"},
		{name: "optional removed, additionalProperties absent: tolerated, the value just has no effect",
			class: ClassNonBreaking, rule: RuleInputOptionalPropertyRemoved, detailHas: "no longer has a declared effect"},
		{name: "optional removed, additionalProperties:true: tolerated",
			newTail: `,"additionalProperties":true`, class: ClassNonBreaking, rule: RuleInputOptionalPropertyRemoved, detailHas: "not rejected"},
		{name: "optional removed, additionalProperties is a schema: tolerated",
			newTail: `,"additionalProperties":{"type":"string"}`, class: ClassNonBreaking, rule: RuleInputOptionalPropertyRemoved, detailHas: "not rejected"},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			req := ""
			...
			c := wantOne(t, got, tc.class, tc.rule, "input.gone")
	...
	t.Run("output optional removal stays unclassified (standing posture)", func(t *testing.T) {
...
func TestEnum_OneChangePerField(t *testing.T) {
	cases := []struct {
		...
		{name: "input: value removed", side: sideInput,
			...
			class: ClassBreaking, rule: RuleInputEnumValueRemoved, removed: []any{"fraudulent"}, added: []any{}},
		{name: "input: value added", side: sideInput,
			before: `["duplicate","fraudulent"]`, aft: `["duplicate","fraudulent","chargeback"]`,
			class: ClassNonBreaking, rule: RuleInputEnumValueAdded, removed: []any{}, added: []any{"chargeback"}},
		{name: "input: value replaced (swap) — one row, breaking", side: sideInput,
			before: `["a","b"]`, aft: `["a","c"]`,
			class: ClassBreaking, rule: RuleInputEnumValueReplaced, removed: []any{"b"}, added: []any{"c"}},
		{name: "output: value removed", side: sideOutput,
			before: `["pending","succeeded","failed"]`, aft: `["pending","succeeded"]`,
			class: ClassBreaking, rule: RuleOutputEnumValueRemoved, removed: []any{"failed"}, added: []any{}},
		{name: "output: value added", side: sideOutput,
			before: `["pending","succeeded"]`, aft: `["pending","succeeded","requires_action"]`,
			class: ClassNonBreaking, rule: RuleOutputEnumValueAdded, removed: []any{}, added: []any{"requires_action"}},
		{name: "output: value replaced (dimhour list_cities kind: region -> state) — one row, not removed + added", side: sideOutput,
			...
			class: ClassBreaking, rule: RuleOutputEnumValueReplaced, removed: []any{"region"}, added: []any{"state"}},
		{name: "output: two removed, one added is still one replaced row", side: sideOutput,
			before: `["a","b","c"]`, aft: `["a","d"]`,
			class: ClassBreaking, rule: RuleOutputEnumValueReplaced, removed: []any{"b", "c"}, added: []any{"d"}},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			c := wantOne(t, Classify(before, after), tc.class, tc.rule, path)
...
func TestPropertyRenamed(t *testing.T) {
	t.Run("input camelCase -> snake_case, required (neon compare_database_schema.branchId): one breaking row", func(t *testing.T) {
		...
		c := wantOne(t, Classify(before, after), ClassBreaking, RuleInputPropertyRenamed, "input.branchId")
	...
	t.Run("input optional twin (neon databaseName -> database_name): still one rename, never removed + added", func(t *testing.T) {
		...
		wantOne(t, Classify(before, after), ClassBreaking, RuleInputPropertyRenamed, "input.databaseName")
	})
	t.Run("kebab-case and PascalCase fold to the same key", func(t *testing.T) {
		...
		wantOne(t, Classify(before, after), ClassBreaking, RuleInputPropertyRenamed, "input.project-id")
	})
	t.Run("output required removed with a twin: output-property-renamed", func(t *testing.T) {
		...
		c := wantOne(t, Classify(before, after), ClassBreaking, RuleOutputPropertyRenamed, "output.refundId")
		if c.Before.(map[string]any)["name"] != "refundId" || c.After.(map[string]any)["name"] != "refund_id" {
			t.Errorf("fragments: %v -> %v", c.Before, c.After)
		}
	})
	t.Run("output OPTIONAL removed with a twin: not a rename — its removal is unclassified, the twin is an optional addition", func(t *testing.T) {
		...
		wantOne(t, Classify(before, after), ClassNonBreaking, RuleOutputOptionalPropertyAdded, "output.logo_url")
	})
	...
	t.Run("an unrelated spelling is a real removal and a real addition (aave reserve -> reserveId)", func(t *testing.T) {
		...
		got := Classify(before, after)
		if len(got) != 2 || got[0].Rule != RuleInputOptionalPropertyRemoved || got[0].Class != ClassBreaking ||
			got[1].Rule != RuleInputOptionalPropertyAdded {
			t.Fatalf("want a breaking optional removal (additionalProperties:false) + an optional addition, got:\n%s", dump(got))
		}
	})
	...
	t.Run("nested: a rename inside items pairs too", func(t *testing.T) {
		...
		wantOne(t, Classify(before, after), ClassBreaking, RuleOutputPropertyRenamed, "output.rows.items.cityKey")
	})
}
