...
func TestFindingSyncConfigOff(t *testing.T) {
	...
	r.start(t)
	connectKeyOnly(t, r)
	r.ext.cfg.FindingSync, r.ext.cfg.DisplayNameSync = false, false
...
//
// `finding_sync` and `display_name_sync` gate the two legs of the ONE ticker
...
func runSyncLoop(t *testing.T, findingSync, displayNameSync bool) (findings, directory int, started bool) {
	...
	r.cp.mu.Unlock()
	r.ext.cfg.FindingSync, r.ext.cfg.DisplayNameSync = findingSync, displayNameSync
	...
	if started {
		...
		for (findingSync && r.cp.findingsCallCount() == 0) || (displayNameSync && r.cp.directoryCallCount() == 0) {
			if time.Now().After(deadline) {
				t.Fatalf("enabled leg never reached the wire (finding_sync=%v display_name_sync=%v; findings=%d directory=%d)",
...
func TestSyncSwitchesGateLegsIndependently(t *testing.T) {
	cases := []struct {
		...
		{"display_name_sync off — findings still post, no directory GET", true, false, true, false, true},
...
// TestDisplayNameSyncDefaultOn pins the factory default: an omitted
// display_name_sync key means ON (CONTRACTS §8), so splitting the switch off
// finding_sync changed nobody's behaviour.
func TestDisplayNameSyncDefaultOn(t *testing.T) {
	cfg := createDefaultConfig().(*Config)
	if !cfg.DisplayNameSync {
		t.Fatal("display_name_sync must default to true")
	}
	// And it survives a config file that never mentions the key: unmarshalling
	// a flanjui block WITHOUT display_name_sync must leave the default alone.
	...
	if !cfg2.DisplayNameSync {
		t.Fatal("an absent display_name_sync key must stay true — finding_sync: false must not disable name refreshes")
	}
}
